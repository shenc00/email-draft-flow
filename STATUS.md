...
### Pending Tasks
- **Blocker: fix agent-browser tooling** before any further build steps — run `agent-browser doctor --fix` outside this session, or confirm reinstall. Suspected cause: BD tenant TLS interception/proxy (same pattern as prior node/npm issue, see memory `bd-tls-interception-node-downloads`), or a stale daemon socket.
